...
export default function Dashboard() {
  ...
  const cargar = async () => {
    ...
    if (invs?.length > 0) {
      ...
      const { data: gruposInv } = await supabase.from("grupos").select("*").in("id", grupoIds);
      const { data: perfilesInv } = await supabase.from("perfiles").select("*").in("id", deIds);
      setInvitaciones(invs.map(inv => ({
        ...inv,
    ...
    const gruposConInfo = await Promise.all((gruposData || []).map(async g => {
      ...
      const { data: miembrosGrupo } = await supabase.from("miembros").select("*").eq("grupo_id", g.id);
      const { data: notifs } = await supabase.from("notificaciones").select("*").eq("grupo_id", g.id).order("creado_en", { ascending: false }).limit(1);
      return { ...g, juntadas: juntadas || [], miembros: miembrosGrupo || [], ultimaNotif: notifs?.[0] };
    }));
  ...
  const salirDelGrupo = async (grupoId) => {
    const { data: { user } } = await supabase.auth.getUser();
    await supabase.from("miembros").delete().eq("grupo_id", grupoId).eq("user_id", user.id);
  ...
  return (
    <main style={{ background: "var(--bg)", minHeight: "100vh" }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: "24px" }}>
          <div className="fade-in" style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "20px", padding: "28px", maxWidth: "360px", width: "100%" }}>
            ...
            <p style={{ fontSize: "13px", color: "var(--text-muted)", marginBottom: "24px", lineHeight: 1.6 }}>
              Vas a salir de <strong style={{ color: "var(--text)" }}>{grupos.find(g => g.id === confirmandoSalir)?.nombre}</strong>. Tu disponibilidad se va a borrar.
            </p>
            <div style={{ display: "flex", gap: "10px" }}>
              <button onClick={() => setConfirmandoSalir(null)} style={{ flex: 1, padding: "12px", background: "var(--surface-2)", border: "1px solid var(--border)", color: "var(--text)", borderRadius: "12px", fontFamily: "Syne, sans-serif", fontSize: "14px", fontWeight: 600, cursor: "pointer" }}>Cancelar</button>
              <button onClick={() => salirDelGrupo(confirmandoSalir)} style={{ flex: 1, padding: "12px", background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.2)", color: "#F87171", borderRadius: "12px", fontFamily: "Syne, sans-serif", fontSize: "14px", fontWeight: 700, cursor: "pointer" }}>Salir</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <button onClick={() => router.push("/amigos")} style={{ position: "relative", fontSize: "12px", background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text)", padding: "5px 14px", borderRadius: "8px", cursor: "pointer", fontWeight: 600 }}>
              ...
              {solicitudesPendientes > 0 && (
                <span style={{ position: "absolute", top: "-6px", right: "-6px", width: "16px", height: "16px", background: "#F87171", borderRadius: "50%", fontSize: "10px", fontWeight: 700, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {solicitudesPendientes}
                </span>
              )}
            </button>
            <button onClick={cerrarSesion} style={{ fontSize: "12px", background: "none", border: "1px solid var(--border)", color: "var(--text-muted)", padding: "5px 12px", borderRadius: "8px", cursor: "pointer" }}>Salir</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: "560px", margin: "0 auto", padding: "32px 20px" }}>
        ...
        {/* Stats */}
        {grupos.length > 0 && (
          <div className="fade-up s0" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px", marginBottom: "28px" }}>
            {[
              ...
              <div key={s.label} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "14px", padding: "16px", textAlign: "center" }}>
                ...
                <p style={{ fontSize: "11px", color: "var(--text-muted)", fontWeight: 600, letterSpacing: "0.04em", textTransform: "uppercase" }}>{s.label}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {/* Acciones */}
        <div className="fade-up s1" style={{ display: "flex", gap: "10px", marginBottom: modo ? "0" : "28px" }}>
          <button onClick={() => setModo(modo === "crear" ? null : "crear")} style={{ flex: 1, padding: "13px", background: "var(--accent)", color: "#0C0C0F", border: "none", borderRadius: "12px", fontFamily: "Syne, sans-serif", fontSize: "14px", fontWeight: 700, cursor: "pointer", opacity: modo === "unirse" ? 0.5 : 1, transition: "opacity 0.2s" }}>
            + Crear grupo
          </button>
          <button onClick={() => setModo(modo === "unirse" ? null : "unirse")} style={{ flex: 1, padding: "13px", background: "var(--surface)", color: "var(--text)", border: "1px solid var(--border)", borderRadius: "12px", fontFamily: "Syne, sans-serif", fontSize: "14px", fontWeight: 700, cursor: "pointer", opacity: modo === "crear" ? 0.5 : 1, transition: "opacity 0.2s" }}>
            Unirme con codigo
          </button>
        </div>

        {modo && (
          <div className="fade-in" style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "16px", padding: "20px", margin: "12px 0 24px" }}>
            <p style={{ fontSize: "12px", fontWeight: 700, color: "var(--text-muted)", letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: "12px" }}>
              {modo === "crear" ? "Nombre del grupo" : "Codigo de invitacion"}
            </p>
            <div style={{ display: "flex", gap: "10px" }}>
              <input
                type="text"
                placeholder={modo === "crear" ? "Ej: Los pibes" : "Ej: A1B2C3"}
                value={modo === "crear" ? nombreGrupo : codigoUnirse}
                onChange={e => modo === "crear" ? setNombreGrupo(e.target.value) : setCodigoUnirse(e.target.value)}
                onKeyDown={e => e.key === "Enter" && (modo === "crear" ? crearGrupo() : unirseGrupo())}
                style={inputStyle}
                autoFocus
              />
        ...
        {grupos.length === 0 ? (
          <div className="fade-up s2" style={{ textAlign: "center", padding: "64px 0", color: "var(--text-muted)" }}>
            <p style={{ fontSize: "40px", marginBottom: "16px" }}>👥</p>
            <p style={{ fontFamily: "Syne, sans-serif", fontSize: "16px", fontWeight: 700, color: "var(--text)", marginBottom: "8px" }}>Sin grupos todavia</p>
            <p style={{ fontSize: "13px", lineHeight: 1.6 }}>Crea uno o unite con un codigo<br />para empezar a coordinar.</p>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            {grupos.map((g, i) => {
              ...
              const progreso = siguiente ? ((g.juntadas.length - nivel.min) / (siguiente.min - nivel.min)) * 100 : 100;
              return (
                <div key={g.id} className={`fade-up s${Math.min(i + 2, 5)}`}
                  style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "16px", padding: "20px", transition: "border-color 0.2s, transform 0.2s", cursor: "pointer" }}
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "14px" }}>
                    <div onClick={() => router.push(`/grupo/${g.id}?miembro=${perfil.nombre}`)} style={{ flex: 1 }}>
                      <h2 style={{ fontFamily: "Syne, sans-serif", fontSize: "16px", fontWeight: 700, color: "var(--text)", marginBottom: "4px" }}>{g.nombre}</h2>
                      <p style={{ fontSize: "12px", color: "var(--text-muted)" }}>
                        {g.miembros.length} miembros · <span style={{ color: "var(--accent)", fontWeight: 700, letterSpacing: "0.06em" }}>{g.codigo}</span>
                      </p>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      <span style={{ fontSize: "24px" }}>{nivel.emoji}</span>
                      <button
                        onClick={e => { e.stopPropagation(); setConfirmandoSalir(g.id); }}
                        style={{ fontSize: "11px", background: "none", border: "1px solid var(--border)", color: "var(--text-muted)", padding: "5px 10px", borderRadius: "8px", cursor: "pointer", transition: "all 0.2s" }}
